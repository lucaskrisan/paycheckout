  ...
  const handleSubmit = async () => {
    ...
    try {
      if (paymentMethod === "pix") {
        ...
        if (data?.payment_id) { toast.success("Pagamento processado com sucesso!"); trackPurchase(finalAmount, "BRL", data.payment_id); await markPurchased(); navigate(`/checkout/sucesso?product=${encodeURIComponent(product.name)}&method=credit_card&email=${encodeURIComponent(customer.email)}`); }
