...
const IntegrationsSection = () => (
  <section className="relative z-10 py-28 lg:py-36 overflow-hidden">
    ...
    <div className="container max-w-5xl mx-auto px-6 text-center">
      ...
      <div className="grid grid-cols-4 md:grid-cols-6 gap-5 max-w-2xl mx-auto">
        {integrations.map((item, i) => (
          ...
          >
            <div
              className="w-16 h-16 rounded-2xl bg-[#141417] border border-white/[0.06] flex items-center justify-center group-hover:border-white/[0.12] transition-all duration-500 relative overflow-hidden"
            >
              <div
                className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{ background: `radial-gradient(circle at center, ${item.color}10 0%, transparent 70%)` }}
              />
              <item.icon className="w-6 h-6 opacity-50 group-hover:opacity-90 transition-opacity duration-300" style={{ color: item.color }} />
            </div>
            <span className="text-[9px] text-[#5A5A65] font-semibold uppercase tracking-wider group-hover:text-[#8A8A95] transition-colors">
              {item.label}
            </span>
          </motion.div>
        ))}
      ...
      <motion.p
        className="text-[13px] text-[#6A6A75] mt-14 font-light"
        initial={{ opacity: 0 }}
