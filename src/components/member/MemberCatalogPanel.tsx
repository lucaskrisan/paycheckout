...
const MemberCatalogPanel = memo(function MemberCatalogPanel({ courses, customerId, open, onClose }: Props) {
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          style={{ background: "hsl(220 18% 8%)", borderColor: "hsl(220 15% 12%)" }}
        >
          <div className="max-w-[1440px] mx-auto px-3 sm:px-6 py-6 sm:py-8">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <div>
                ...
                <p className="text-[hsl(220,10%,45%)] text-xs sm:text-sm mt-1">Expanda seu conhecimento</p>
              </div>
              <button onClick={onClose} className="text-[hsl(220,10%,45%)] hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
                    ...
                    <div className="relative h-32 sm:h-36 overflow-hidden">
                      ...
                      {c.hasAccess && (
                        <div className="absolute top-2 right-2">
                          <div className="px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-[hsl(145,65%,42%)] text-white">Liberado</div>
                        </div>
                      )}
                    </div>
                    <div className="p-4">
                      ...
                      {c.hasAccess ? (
                        <button
                          onClick={() => {
                            supabase
                              ...
                              .single()
                              .then(({ data }) => {
                                if (data) window.location.href = `/membros?token=${data.access_token}`;
                              });
                          }}
                        ...
                        <button
                          ...
                          style={{ backgroundImage: "linear-gradient(135deg, hsl(45,93%,50%), hsl(35,90%,45%))", color: "hsl(220,20%,10%)" }}
                        >
