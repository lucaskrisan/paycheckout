...
const MemberSidebarContent = memo(function MemberSidebarContent({
  modules,
  activeModuleId,
  onToggleModule,
  activeLesson,
    ...
    <div className="space-y-2">
      {modules.map((mod, modIndex) => {
        ...
        return (
          ...
          >
            <button
              onClick={() => onToggleModule(isOpen ? null : mod.id)}
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div
                  ...
                  style={{
                    background: modCompleted === modTotal && modTotal > 0 ? "hsl(145,65%,42%)" : "hsl(220,18%,16%)",
                    color: modCompleted === modTotal && modTotal > 0 ? "white" : "hsl(220,10%,50%)",
                  }}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="text-[hsl(220,10%,40%)] text-xs">{modCompleted}/{modTotal} aulas</span>
                </div>
              </div>
                    ...
                    {mod.lessons.map((lesson) => {
                      ...
                      return (
                        <button
                          ...
                          onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.background = "hsl(220,16%,14%)"; }}
                          onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.background = "transparent"; }}
                        >
                          {isCompleted ? (
                            <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: isActive ? "white" : "hsl(145,65%,50%)" }} />
                          ) : (
      ...
      {otherCoursesCount > 0 && (
        ...
        >
          ...
          <div className="text-left">
            ...
            <span className="text-[hsl(220,10%,40%)] text-xs">{otherCoursesCount} disponíveis</span>
          </div>
        </button>
      )}
    </div>
