...
Deno.serve(async (req) => {
  ...
  try {
        ...
        if (recoverCust?.email) {
          ...
          if (recoveredCount && recoveredCount > 0) {
            try {
              ...
              console.error('[asaas-webhook] cart.recovered webhook error (non-blocking):', whErr);
            }
          }
        }
      } catch (recoverErr) {
        console.error('[asaas-webhook] Cart recovery mark error (non-blocking):', recoverErr);
      }
    ...
    if ((event === 'PAYMENT_CONFIRMED' || event === 'PAYMENT_RECEIVED') && orderData?.user_id && orderData?.customer_id) {
      try {
        ...
        if (custWa?.phone) {
          ...
          }).catch(e => console.error('[asaas-webhook] whatsapp-dispatch error:', e));
        }
      } catch (waErr) {
        console.error('[asaas-webhook] WhatsApp dispatch error (non-blocking):', waErr);
      }
    }
