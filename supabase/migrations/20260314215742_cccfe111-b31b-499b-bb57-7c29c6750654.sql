...
BEGIN
  ...
  -- Super admins are exempt from platform fees
  SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = NEW.user_id AND role = 'super_admin') INTO _is_super;
  IF _is_super THEN
  ...
  IF _fee <= 0 THEN NEW.platform_fee_amount := 0; RETURN NEW; END IF;

  NEW.platform_fee_amount := ROUND(_fee, 2);

  INSERT INTO public.billing_accounts (user_id) VALUES (NEW.user_id) ON CONFLICT (user_id) DO NOTHING;

  UPDATE public.billing_accounts SET balance = balance + ROUND(_fee, 2), updated_at = now()
  WHERE user_id = NEW.user_id RETURNING * INTO _account;
  ...
  IF _account.balance > _account.credit_limit THEN
    UPDATE public.billing_accounts SET blocked = true, updated_at = now() WHERE user_id = NEW.user_id;
  END IF;
