-- 1. Adicionar coluna pixel_id (nullable para preservar dados antigos)
ALTER TABLE public.pixel_events
ADD COLUMN IF NOT EXISTS pixel_id text;

-- 2. Backfill: para produtos com apenas 1 pixel, popular o pixel_id
WITH single_pixel_products AS (
  SELECT product_id, MIN(pixel_id) AS pixel_id
  FROM public.product_pixels
  GROUP BY product_id
  HAVING COUNT(*) = 1
)
UPDATE public.pixel_events pe
SET pixel_id = spp.pixel_id
FROM single_pixel_products spp
WHERE pe.product_id = spp.product_id
  AND pe.pixel_id IS NULL;
...
-- 4. Atualizar RPC: agrupar por pixel_id + last_event_at em tempo real + detectar duplicatas
...
BEGIN
  ...
  -- Detecta produtos com pixels duplicados (2+ pixels no mesmo produto)
  ...
  -- Eventos agrupados por pixel_id (preciso) — fallback para product_id quando pixel_id é NULL legacy
  events_window AS (
    SELECT
      COALESCE(pe.pixel_id, '__legacy__') AS pixel_key,
      pe.product_id,
      pe.event_name,
      pe.source,
      COUNT(*) AS cnt,
    ...
    WHERE pe.created_at >= _since
      AND pe.product_id IN (SELECT product_id FROM pixels)
    GROUP BY COALESCE(pe.pixel_id, '__legacy__'), pe.product_id, pe.event_name, pe.source
  ),
  pixel_event_summary AS (
    ...
    FROM pixels px
    LEFT JOIN events_window ew
      ON ew.product_id = px.product_id
      AND (ew.pixel_key = px.pixel_id OR ew.pixel_key = '__legacy__')
    GROUP BY px.id
  ),
  emq_window AS (
    SELECT
