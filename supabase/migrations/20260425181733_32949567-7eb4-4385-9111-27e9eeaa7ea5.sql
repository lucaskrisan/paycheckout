CREATE OR REPLACE FUNCTION public.list_orders_paginated(
  ...
  p_status_filter text DEFAULT 'approved'::text,
  p_period text DEFAULT 'all'::text,
  p_product_id uuid DEFAULT NULL::uuid,
  p_payment_methods text DEFAULT NULL::text,
  p_sale_type text DEFAULT 'all'::text,
  p_currency text DEFAULT NULL::text,
  p_product_type text DEFAULT 'all'::text,
  p_subscription_filter text DEFAULT NULL::text,
  p_utm_search text DEFAULT NULL::text,
  p_affiliate_search text DEFAULT NULL::text,
  p_offer_filter text DEFAULT NULL::text
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
