-- RPC para paginação real de pedidos com filtros e contagem total no servidor.
-- Retorna JSONB com { rows, total_count, total_amount } para evitar dois roundtrips.
...
CREATE OR REPLACE FUNCTION public.list_orders_paginated(
  ...
  p_status_filter text DEFAULT 'approved', -- 'approved' | 'all' | comma-separated list
  p_period text DEFAULT 'all',             -- 'all' | 'today' | '7d' | '30d' | '90d'
  p_product_id uuid DEFAULT NULL,
  p_payment_methods text DEFAULT NULL,     -- comma-separated
  p_sale_type text DEFAULT 'all'           -- 'all' | 'front' | 'with_bumps' | 'upsell'
)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  ...
  _rows jsonb;
BEGIN
  -- Defesa em profundidade: usuário só pode ler seus próprios pedidos
  ...
  -- Status: 'approved' expande para paid/approved/confirmed; 'all' = todos
  ...
  ELSE
    ...
    -- Expande "paid" para incluir aprovados/confirmados; "refused" para incluir failed
  ...
  -- CTE com todos os filtros aplicados
  WITH filtered AS (
    SELECT o.*
  ...
  page AS (
    SELECT f.*,
           ...
           jsonb_build_object('name', p.name) AS product_obj
    FROM filtered f
